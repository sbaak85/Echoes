...
export const CraftingWorkbench=forwardRef<StarshipInteractionMenuController,Props>(function CraftingWorkbench(props,ref){
 ...
 function add(id:string){
   ...
   if(validAllocated.has(id)){setNotice("這項素材已投入，可點中央格子取回");return;}
 ...
 return <div ref={root} className="crafting-workbench starship-interaction-menu" data-owner={owner} data-control-mode={props.controlMode} data-input-mode={props.inputMode}
  ...
  <main className="craft-stage" role="dialog" aria-modal="true" aria-label="道具合成系統" style={{"--scale":scale} as CSSProperties}>
   ...
   <div className="workspace">
    <section className="panel inventory"><div className="panel-head"><div><small>01 / MATERIAL STORAGE</small><h2>素材背包</h2></div><span className="count">{owned.length} 種物品</span></div>
     ...
     <div id="craft-inventory" className="inventory-grid">{materials.slice(activePage*12,(activePage+1)*12).map(entry=>{
       ...
       return button(`material-${entry.id}`,<><Category entry={entry}/><ItemArt key={entry.id} entry={entry}/><span className="quantity">{(props.inventory[entry.id]||0)-(added?need![1]:0)}</span><span className="item-name">{entry.name}</span></>,()=>add(entry.id),{className:`item ${need?"is-needed":""} ${added?"is-added":""}`,disabled:locked,aria:`${entry.name}，${categoryNames[category(entry)]}，持有 ${props.inventory[entry.id]||0}，點選投入`});
    ...
    <section className={`panel recipe ${ready||phase==="crafted"?"all-ready":""}`}><div className="panel-head"><div><small>02 / ASSEMBLY CHAMBER</small><h2>合成配方</h2></div><span id="craft-progress" className="count">{progress} / {current.req.length} {phase==="crafted"?"已完成":"已投入"}</span></div>
     ...
     <div className="machine-zone"><div className="feeds"><i/><i/><i/></div><div className="platform"/><img className="machine" src={assetUrl("ui/power-devices/workbench-core-transparent.png")} alt="製作工作台"/><div className="machine-caption">FABRICATION UNIT <b>{phase==="crafted"?"COMPLETE":ready?"READY":"STANDBY"}</b></div></div>
    ...
    <section className="panel output"><div className="panel-head"><div><small>03 / {locked?"CRAFTING RESULT":"RECIPE LIBRARY"}</small><h2>{locked?"製作結果":"可製作物品"}</h2></div><span className="count">{locked?"× 1":CRAFTING_RECIPES.length}</span></div>
