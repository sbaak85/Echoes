...
type View = "main" | "sleep" | "craft" | "closed";
...
export default function InteractionMenuPreview() {
  ...
  const row = (index: number, imageLabel: string, title: string, detail: string, action: () => void, disabled = false) => (
    <button className={`im-row ${selected === index && !disabled ? "is-selected" : ""}`} disabled={disabled}
      ...
      <span className="im-option-media" aria-hidden="true"><span>{imageLabel}</span><small>IMAGE PLACEHOLDER</small></span>
  ...
  return <main className="im-stage" data-input-mode={inputMode}
    ...
    <div className="im-preview-label">互動選單 · 排版預覽</div>
    ...
    <section ref={panel} className={`im-panel im-view-${view}`} role={view === "closed" ? "region" : "dialog"} aria-modal={view === "closed" ? undefined : true} aria-labelledby="im-title">
      <div className="im-hero">
        <div className="im-hero-media" aria-hidden="true"><span>情境圖片容器</span><small>SCENE IMAGE · 圖片待補</small></div>
        <header>
          <div className="im-emblem" aria-hidden="true"><i /><b><span>✦</span></b><i /></div>
          <span className="im-kicker">{view === "sleep" ? "REST & RECOVER" : view === "craft" ? "CRAFTING STATION" : view === "closed" ? "STARSHIP SHELTER" : "STARSHIP INTERACTION"}</span>
          <h1 id="im-title">{view === "sleep" ? "休息與睡眠" : view === "craft" ? "製作道具／食物" : view === "closed" ? "已返回飛船" : "飛船休息艙"}</h1>
          {view !== "main" && (
            <p>{view === "sleep" ? "選擇休息時間，為下一段旅程做好準備。" : view === "craft" ? "利用收集到的資源，準備下一次探索。" : notice || "你可以繼續探索，或再次開啟功能選單。"}</p>
          )}
        </header>
      </div>
      <div className="im-options">
        {view === "main" && <>
          {row(0, "睡眠圖片", "睡覺", "恢復狀態 · 選擇休息時間", () => changeView("sleep"))}
          {row(1, "製作圖片", "製作道具／食物", "加工資源 · 準備探索補給", () => changeView("craft"))}
          {row(2, "維修圖片", "維修飛船", "修復受損系統與艙體", () => {}, true)}
          <button className={`im-row im-cancel ${selected === 2 ? "is-selected" : ""}`}
            onFocus={() => setSelected(2)} onMouseEnter={event => { setSelected(2); event.currentTarget.focus({ preventScroll: true }); }} onClick={back}>
            <span className="im-option-media" aria-hidden="true"><span>返回圖片</span><small>IMAGE PLACEHOLDER</small></span>
            <span className="im-copy"><strong>取消／返回</strong><small>關閉選單，不執行互動</small></span>
          </button>
        </>}
        {view === "sleep" && <>{row(0, "八小時圖片", "睡滿 8 小時", "從現在起，完整休息八個小時", () => chooseSleep("睡滿8小時"))}{row(1, "清晨圖片", "睡到明天 06 點", "休息至明天清晨 06:00", () => chooseSleep("睡到明天06點"))}</>}
        {view === "craft" && <div className="im-craft-empty"><span>⚒</span><h2>製作工作台</h2><p>道具與食物的配方將在這裡顯示。</p><small>介面預覽 · 配方與製作功能尚未接入</small></div>}
        {view === "closed" && row(0, "休息艙圖片", "開啟功能選單", "返回飛船休息艙", () => { setNotice(""); changeView("main"); })}
      </div>
      {view !== "closed" && view !== "main" && <button className={`im-back ${selected === (view === "craft" ? 0 : 2) ? "is-selected" : ""}`} onFocus={() => setSelected(view === "craft" ? 0 : 2)} onClick={back}><span>↶</span>返回功能選單</button>}
