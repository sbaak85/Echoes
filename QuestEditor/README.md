...
- OBJ「啟用方式」除了立即／事件啟用，也可選「OBJ啟用後啟用」或
  「OBJ核取後啟用」，並在「啟用事件 ID／OBJ ID」填入來源 OBJ。編輯器會
  改列 OBJ 清單供選取；來源狀態發生時，目標 OBJ 依自己的啟動延遲解鎖。
- 「複合道具收集」可設定「複合道具判定模式」：預設「全部道具達標」；
  選「任選 N 種」時，外層「需求數量」就是 N。每種道具必須累計取得集合內
  指定的數量才算一種，同一 Item ID 不重複計種。例如五種各 ×1、N=2，
  取得任意兩種即可完成。N 不可超過集合內不同道具種類數，集合不可重複 ID。
- 「互動開始／互動成功」可用「指定互動 ID 清單」累積不同 Interaction；
  同一互動點只計一次，達到「需求數量」後才完成。
- 新增、複製、刪除、排序。
- 「完成場景轉移」以抵達目標場景後的事件計數；判定目標 ID 填入 Scene ID。
  可選填來源場景及來源出口，出口清單只列出該來源場景的出口。
  未填目標場景不會判定；碰觸出口、轉移失敗、讀檔出現在該場景均不計入。
