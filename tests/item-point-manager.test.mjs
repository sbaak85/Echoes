...
test("Scene 唯一 ItemPoint ID 會使用獨立的生成與拾取進度", () => {
  const sceneA = {
    sceneId: "Scene_A",
    id: "scenea-item-point-001",
    label: "Scene A Item",
    x: 1,
    y: 1,
    itemId: "R0001",
  ...
  assert.deepEqual(progress.onceCollectedIds, ["Scene_A:scenea-item-point-001"]);
});

test("舊版 ItemPoint 拾取進度會遷移到 Scene 唯一 ID", () => {
  assert.deepEqual(normalizeItemPointProgress({
    onceCollectedIds: [
      "Scene_3:item-point-001",
      "Scene_3:scene3-item-point-001",
    ],
    dailyCollectedCycles: {
      "Scene_2:item-point-001": 4,
      "Scene_2:scene2-item-point-001": 7,
    },
  }), {
    onceCollectedIds: ["Scene_3:scene3-item-point-001"],
    dailyCollectedCycles: {
      "Scene_2:scene2-item-point-001": 7,
    },
  });
