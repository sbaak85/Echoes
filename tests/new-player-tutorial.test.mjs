...
test("遮罩為 70% 黑、柔邊開孔，提示框會呼吸發亮", () => {
  ...
  assert.doesNotMatch(styles, /\.new-player-tutorial-actions\s*\{[^}]*animation:/);
  assert.match(overlaySource, /new-player-tutorial-actions[\s\S]*new-player-tutorial-action-copy/);
  assert.match(styles, /\.new-player-tutorial-action-copy\s*\{[^}]*animation:\s*new-player-tutorial-prompt-breathe 1\.25s/);
