...
test("craft view connects the item workbench and preserves the cooking placeholder", () => {
  const craftView = menuSource.slice(menuSource.indexOf('{view === "craft" ? <>'), menuSource.indexOf('{view === "repair" ? <div'));
  assert.equal((craftView.match(/className="im-craft-empty"/g) ?? []).length, 1);
  assert.ok(craftView.indexOf("製作工作台") < craftView.indexOf("料理工作台"));
  assert.match(craftView, /changeView\("workbench"\)/);
  assert.match(craftView, /食物與飲品的配方將在這裡顯示/);
  assert.match(craftView, /<button.*data-starship-menu-index=\{0\}/);
});
