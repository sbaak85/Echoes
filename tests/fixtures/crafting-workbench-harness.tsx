...
import { AudioEventManager } from "../../app/audio-event-manager";
...
function Harness(){
 ...
 useEffect(()=>{audio.current=new AudioEventManager();return()=>{audio.current?.dispose();audio.current=null;};},[]);
 ...
 <output aria-label="Hover audio log">{audioLog.join(" → ")}</output>
 ...
 onWorkbenchHoverAudio={event=>{if(event)setAudioLog(log=>[...log,event]);void audio.current?.playWorkbenchHover(event).catch(()=>{});}}
 onWorkbenchOpenAudio={event=>{setAudioLog(log=>[...log,event]);void audio.current?.play(event).catch(()=>{});}}
