...
- 原始素材與「遊戲 MP3 路徑」右側的 `📂` 可在 Windows 檔案總管中
  選取第一個 MP3，`▶` 可直接試聽；播放中再次按下即可停止。
- 原始素材路徑可留空；未填寫時，該欄位的檔案總管與預覽按鈕會自動停用。
- 更換素材時，將瀏覽器需要的 MP3 放入 `public/audio/`，再修改該事件的
  `sourceAssetPaths` 與 `sources`。
- 管理視窗新增「BGM 管理」頁籤；可在素材庫建立多首 Track，再用控制規則
  依 Quest／Stage／OBJ／小遊戲／章節／場景／特殊事件調音量、靜音或交叉換曲。
  規則解除後可續播、重播或回預設曲，且只在狀態改變時重算。
- 「Line SE 管理」頁籤可將任意對話 Line ID 綁定音效，設定音量、延遲、
  FadeIn／FadeOut、Loop，以及切到下一句時自然播完（預設）或淡出停止。
